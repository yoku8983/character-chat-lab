...
> 最終更新: 2026-07-03（Fable 5 セッションからの引継ぎ文書を兼ねる）

## 現在地

- MVP1・MVP2 完了。Azure Container Apps 本番デプロイ済み（libSQL/Turso + スケールゼロ）
- **Issue #20「ペルソナデータ設計・コンテキスト戦略・コスト最適化の総合改善」は 2026-07-03 にクローズ**（T1〜T7 完了。仕様書反映 PR #33 マージ済み）
- 後継の **Issue #34「評価の高精度化とペルソナ設計の実証改善」** に着手フェーズ。Issue #34 本文に「実装コンテキスト（引き継ぎ）」セクションを追記済みで、既存評価基盤の構成・T1 改修ポイント（`scripts/eval/judge.ts` / `report.ts`）・運用ルールをコード実査済みで記載している。**次セッションはまず Issue #34 を読むこと**

## 次にやること（Issue #34 のタスク、優先順）

1. **T1: 正解データセット評価と参照付き Judge**（最高）— 全体のボトルネック。最優先
2. **T2: 品質回帰の継続トラッキング**（高）— T1 完了が前提
3. **T3: ペルソナスキーマの実証改善**（中）— T1 の尖った評価で A/B 検証しながら進める

タスクの詳細・チェックボックスは Issue #34 を参照し、完了したら Issue 側のチェックを更新すること。

<details>
<summary>Issue #20（T1〜T7）完了時点の旧タスクリスト（クローズ済み・折りたたみ）</summary>
...
7. 動的コンテキスト選択は**封印**（解除条件は Issue #20 参照。実装しないこと。T3 の関連タスクにも影響しうるので着手前に確認）

→ Issue #20 の対策タスク T1〜T7 は一巡完了（T6 の会話例拡充・知識エピソード化のみ意図的に保留、Issue #34 の T3 に引き継ぎ）。
...
- 2026-07-03: **Issue #20 クローズ**（T1〜T7 完了。見送り項目〔T6 の会話例拡充・知識エピソード化〕は再開条件付きで後継 Issue #34 に引き継ぎ）
  - ドキュメント反映 PR #33 マージ済み（`docs/issue20-improvements-spec.md` 新設。仕様・実測事実を集約）
  - 次の作業: **Issue #34「評価の高精度化とペルソナ設計の実証改善」**（本文に「実装コンテキスト（引き継ぎ）」セクションを追記済み。次セッションはまず Issue #34 を読むこと）
- 2026-07-03: **T1〜T7 一巡後の設計レビュー（Fable）— 実装は設計意図どおり。残留指摘 3 点を記録**
  - **【方法論】T5 スイープの Gemini 行は Judge 規定違反**: Judge=`gemini-3.1-flash-lite` のまま同モデル自身も被評価したため、Gemini 行の judge スコア（全5.0）は自己選好バイアスの疑いあり（Issue #20 の「Judge を被評価モデルと同系にしない」に抵触）。**採用決定（DeepSeek Flash 継続）への影響はない**（DeepSeek は別系統 Judge で採点済み、Gemini はコストで不採用）。マーカー出現率 100%/100% は正規表現ベースで有効。**将来 Gemini を premium 選択肢として再評価する際は Judge を別系統（DeepSeek 系等）に替えること**
  - **【構造特性】T2 のメッセージ数カットは長い会話で履歴部分のキャッシュを毎ターン壊す**: 上限30超の会話では毎ターン窓が2メッセージずつスライドし、system+few-shot より後ろのプレフィクスは毎ターン不一致になる（スライディングウィンドウの本質的性質・設計の織り込み済み）。帰結として**長い会話ではキャッシュヒットは system+few-shot 部分に縮退する**。T1 ログで長会話のヒット率が低く見えてもバグではない
  - **【軽微】T4 Judge の失敗時スコア 0 が集計を歪め得る**: `judgeConversation` は失敗時に全スコア 0（正常レンジ 1〜5 の外）を返し aggregate の単純平均に混入する。1本失敗すると平均が沈み品質劣化に見える。今回のランは skip 0 で実害なし。次回ハーネス改修時に「judge 失敗シナリオは平均から除外」を入れる（一行修正）
