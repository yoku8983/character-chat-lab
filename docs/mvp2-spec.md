...
- 抽出: 直近20メッセージを LLM に送り、`{content, importance}[]` を JSON で返却（Issue #20 で既存記憶との重複登録を防止。[Issue #20 仕様書](issue20-improvements-spec.md) T3 参照）
...
personas (id TEXT PK, name, definition JSON, source 'yaml'|'db', profile_image, created_at, updated_at)
```

> **注記（Issue #20 以降の追加）:** 上記に加え、実測 usage を記録する `usage_log` テーブル、YAML 再シードの差分判定用 `seeded_personas`（`content_hash`）が追加されている。詳細は [Issue #20 仕様書](issue20-improvements-spec.md) を参照。
